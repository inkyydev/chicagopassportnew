...
export default function VisasArchive({ title, description, data }) {
  const location = useLocation();

  let slug = "/visas";

  if (location.pathname === "/e-visas") {
    slug = "/e-visas";
  }
  return (
    <>
      ...
      >
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-[20px]">
            {data.map((visa) => (
              <Link to={`${slug}/${visa.slug}`} key={visa.id} className="block">
                <div
                  className="
            ...
            <div
              className="other-div-all col-span-1 md:col-span-2 bg-[#122241] rounded-[20px] flex flex-col justify-end items-start p-[39px_42px] bg-[url('../../../assets/other-bg.png')] bg-no-repeat bg-top bg-containmax-[1024px]:p-[30px_25px] max-[767px]:p-[30px_20px]
        "
            >
              <h3
